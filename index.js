    ...
    if (newTask && newTask.trim() !== "") {
      ...
      try {
        const result = await db.query(
          "INSERT INTO list (user_id, task) VALUES ($1, $2)",
          [req.user.id, newTask],
        );
      } catch (err) {
        console.log(err);
...
app.post("/edit", async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    try {
      await db.query(
        "UPDATE list SET task = $1 WHERE id = $2 AND user_id = $3",
        [updatedTask, editItemId, req.user.id],
      );
    } catch (err) {
      console.log(err);
    }
...
app.post("/delete", async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    ...
    try {
      await db.query("DELETE FROM list WHERE id = $1 AND user_id = $2", [
        deleteItemId,
        req.user.id,
      ]);
    } catch (err) {
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: "https://email-scheduler-yuei.onrender.com/auth/google/do",
